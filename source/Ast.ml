...
type expression =
  ...
  | Not (* not *)
  | Fun of builtin
  | Pluck of expression (* pluck(expr) *)
  | Compact (* compact *)
  | Partition of expression (* partition(expr) *)
  | Is_empty (* is_empty *)
  | Is_blank (* is_blank *)
  | Assert of expression * expression option (* assert(expr; msg) *)
  | Debug_msg of expression option (* debug(msg) *)
  | Descend (* descend *)
  | Dive (* dive *)
  | Find_all of expression (* find_all(expr) *)
  | Find_first of expression (* find_first(expr) *)
  | Paths_to of expression (* paths_to(expr) *)
[@@deriving show { with_path = false }]
