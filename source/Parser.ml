...
let make_stream buf =
  let s = { buf; token = Lexer.EOF; start_pos = Lexing.dummy_pos; end_pos = Lexing.dummy_pos } in
  s
...
let advance s =
  ...
  let tok =
    ...
    | Error e ->
        ...
        let err =
          Query_error.lexer_error ~message:e ~input:""
            ~start_pos:start.pos_cnum ~end_pos:stop.pos_cnum
        in
...
let optional_question s =
  match peek s with
  | Lexer.QUESTION_MARK -> advance s; true
  | _ -> false
...
let wrap_optional opt expr =
  if opt then Optional expr else expr
...
and parse_fn_param s =
  match peek s with
  | Lexer.IDENTIFIER name -> advance s; name
  | Lexer.VARIABLE name -> advance s; "$" ^ name
...
and parse_index_number s =
  ...
  | Lexer.SUB ->
      advance s;
      (match peek s with
       | Lexer.INT n -> advance s; -n
       | Lexer.INT64 n -> advance s; Int64.to_int (Int64.neg n)
       | Lexer.BIG_INT n -> advance s; Z.to_int (Z.neg n)
       | Lexer.FLOAT n -> advance s; int_of_float (-.n)
       | _ -> error s "expected number after '-'")
  | Lexer.INT n -> advance s; n
  | Lexer.INT64 n -> advance s; Int64.to_int n
  | Lexer.BIG_INT n -> advance s; Z.to_int n
  | Lexer.FLOAT n -> advance s; int_of_float n
...
and parse_number_literal s =
  ...
  | Lexer.SUB ->
      advance s;
      (match peek s with
       | Lexer.INT n -> advance s; Int (-n)
       | Lexer.INT64 n -> advance s; Int64 (Int64.neg n)
       | Lexer.BIG_INT n -> advance s; Big_int (Z.neg n)
       | Lexer.FLOAT n -> advance s; Float (-.n)
       | _ -> error s "expected number after '-'")
  | Lexer.INT n -> advance s; Int n
  | Lexer.INT64 n -> advance s; Int64 n
  | Lexer.BIG_INT n -> advance s; Big_int n
  | Lexer.FLOAT n -> advance s; Float n
...
and parse_primary s =
  ...
  | Lexer.IF ->
      parse_if s
  | Lexer.FUNCTION name ->
      parse_function_call s name
  | Lexer.IDENTIFIER name ->
      parse_identifier s name
  | Lexer.OPEN_BRACKET ->
      parse_array_construction s
  | Lexer.OPEN_BRACE ->
      parse_object_construction s
  | Lexer.OPEN_PARENT ->
      parse_paren s
  | _ ->
      error s (Printf.sprintf "unexpected token: %s" (Lexer.show_token (peek s)))
...
and parse_dot s =
  ...
  | Lexer.IDENTIFIER k ->
      ...
      if opt then Optional (Key k) else Key k
  | Lexer.OPEN_BRACKET ->
      parse_bracket_access s Identity
  | _ -> Identity
...
and parse_reduce s =
  ...
  expect s Lexer.AS;
  let var = match peek s with
    | Lexer.VARIABLE v -> advance s; v
       ...
       | Lexer.SEMICOLON ->
           ...
           (match peek s with
            ...
            | Lexer.CLOSE_PARENT ->
                ...
                let ast = apply_fn_result s_proxy (Language.map_binary_fn name arg1 arg2) in
                wrap_optional opt ast
