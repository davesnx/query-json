...
let swallow_errors fn ~on_fail =
  ...
  | () ->
      ()
  | effect Runtime_error.Fail _, _ ->
      on_fail ()
  | effect User_error _, _ ->
      on_fail ()
...
let rec substitute_params (params : string list) (args : expression list)
  ...
  | Update (l, r) ->
      Update (sub l, sub r)
  | Alternative (l, r) ->
      Alternative (sub l, sub r)
  | Comma (l, r) ->
      Comma (sub l, sub r)
  | Operation (l, op, r) ->
      Operation (sub l, op, sub r)
  | Assign (l, r) ->
      Assign (sub l, sub r)
  | List e ->
      List (sub_opt e)
  | Object pairs ->
      Object (List.map (fun (k, v) -> (sub k, sub_opt v)) pairs)
  | Optional e ->
      Optional (sub e)
  | Dynamic_access e ->
      Dynamic_access (sub e)
  | Slice_expr (a, b) ->
      Slice_expr (sub_opt a, sub_opt b)
  | If_then_else (c, t, e) ->
      If_then_else (sub c, sub t, sub e)
  | Range (e1, e2, e3) ->
      Range (sub e1, sub_opt e2, sub_opt e3)
  | Reduce (e, pat, init, update) ->
      Reduce (sub e, pat, sub init, sub update)
  | Foreach (e, pat, init, update, extract) ->
      Foreach (sub e, pat, sub init, sub update, sub extract)
  | As (e, pat, body) ->
      As (sub e, pat, sub body)
  | Try (e, h, f) ->
      Try (sub e, sub_opt h, sub_opt f)
  | Fma (e1, e2, e3) ->
...
let rec set_path value path_components new_value =
  ...
  | `String key :: rest -> (
      ...
      | `Assoc fields ->
          ...
          if exists then
            ...
            `Assoc (fields @ [ (key, set_path `Null rest new_value) ])
      | `Null ->
          `Assoc [ (key, set_path `Null rest new_value) ]
      | _ ->
          value
    )
    ...
    -> (
      ...
      | `List items ->
          let rec update_list i = function
            | [] ->
                if i = idx then
                  [ set_path `Null rest new_value ]
                else
                  []
            | x :: xs ->
                if i = idx then
                  set_path x rest new_value :: xs
                else
                  x :: update_list (i + 1) xs
          in
  ...
  | _ :: rest ->
      set_path value rest new_value

let keys ~ctx (json : Json.t) =
  match json with
  | `Assoc list ->
      `List (List.map (fun (k, _) -> `String k) list)
  | `List l ->
      let len = List.length l in
      `List (List.init len (fun i -> `Int64 (Int64.of_int i)))
  | _ ->
      fail_invalid_type ~ctx "keys" json
...
let to_local_time ~ctx json =
  ...
  | `Int64 n ->
      ...
      tm_to_array tm tm.tm_isdst
  | _ ->
      fail_invalid_type ~ctx "to_local_time" json
...
let to_utc ~ctx json =
  ...
  | `Int64 n ->
      ...
      tm_to_array tm false
  | _ ->
      fail_invalid_type ~ctx "to_utc" json
...
let to_unix ~ctx json =
  ...
  | _ ->
      fail_invalid_type ~ctx "to_unix" json
...
let from_date ~ctx json =
  ...
  | _ ->
      fail_invalid_type ~ctx "from_date" json
...
let parse_date_fn ~ctx fmt json =
  ...
  | _ ->
      fail_invalid_type ~ctx "parse_date" json
...
let length ~ctx (json : Json.t) =
  ...
  match json with
  | `List list ->
      `Int64 (Int64.of_int (List.length list))
  | `String s ->
      `Int64 (Int64.of_int (utf8_codepoint_length s))
  | `Assoc obj ->
      `Int64 (Int64.of_int (List.length obj))
  | `Null ->
      `Int64 0L
  | `Int n ->
      `Int64 (Int64.of_int (abs n))
  | `Int64 n ->
      `Int64 (Int64.abs n)
  | `Big_int z ->
      `Big_int (Z.abs z)
  ...
  | `Float f ->
      `Float (Float.abs f)
  | _ ->
      fail_invalid_type ~ctx "length" json
...
let byte_length ~ctx (json : Json.t) =
  ...
  | _ ->
      fail_invalid_type ~ctx "byte_length" json
...
let ceil_op ~ctx json =
  match json with
  | `Float f ->
      `Int64 (Int64.of_float (Float.ceil f))
  | `Int n ->
...
let round_op ~ctx json =
  match json with
  | `Float f ->
      `Int64 (Int64.of_float (Float.round f))
  | `Int n ->
      `Int64 (Int64.of_int n)
  | `Int64 n ->
      `Int64 n
...
let logb_op ~ctx json =
  match json with
  | `Float f ->
      `Float (Float.log2 (Float.abs f) |> Float.floor)
  | `Int n ->
      `Float (Float.log2 (Float.abs (Float.of_int n)) |> Float.floor)
  | `Int64 n ->
...
let min ~ctx (json : Json.t) =
  match json with
  | `List [] ->
      Runtime_error.empty_array "min"
  | `List l ->
      List.fold_left
        (fun acc x ->
          if Json.compare x acc < 0 then
            x
          else
            acc
        )
        (List.hd l) (List.tl l)
  | _ ->
      fail_invalid_type ~ctx "min" json

let max ~ctx (json : Json.t) =
  match json with
  | `List [] ->
      Runtime_error.empty_array "max"
  | `List l ->
      List.fold_left
        (fun acc x ->
          if Json.compare x acc > 0 then
        ...
        (List.hd l) (List.tl l)
  | _ ->
      fail_invalid_type ~ctx "max" json
...
let flatten ~ctx depth (json : Json.t) =
  ...
  | _ ->
      fail_invalid_type ~ctx "flatten" json
...
let unique ~ctx (json : Json.t) =
  ...
  | `List l ->
      ...
      `List (unique [] l)
  | _ ->
      fail_invalid_type ~ctx "unique" json
...
let any ~ctx (json : Json.t) =
  ...
  | _ ->
      fail_invalid_type ~ctx "any" json
...
let all ~ctx (json : Json.t) =
  ...
  | _ ->
      fail_invalid_type ~ctx "all" json
...
let to_entries ~ctx:_ (json : Json.t) =
  ...
  | `Assoc obj ->
      ...
      `List entries
  | _ ->
      Runtime_error.type_mismatch ~value:json "to_entries requires an object"
...
let from_entries ~ctx (json : Json.t) =
  ...
  | _ ->
      fail_invalid_type ~ctx "from_entries" json
...
let get_envs () =
  try
    ...
    |> List.filter_map (fun s ->
        ...
        | k :: rest ->
            Some (k, `String (String.concat "=" rest))
        | [] ->
            None
    )
...
let transpose ~ctx (json : Json.t) =
  ...
  | `List rows ->
      ...
      if List.length lengths <> List.length rows then
        ...
        `List transposed
  | _ ->
      fail_invalid_type ~ctx "transpose" json
  ...
  let rec loop acc =
    if Queue.is_empty queue then
      ...
      let () =
        match current with
        | `List items ->
            List.iter (fun item -> Queue.push item queue) items
        | `Assoc fields ->
            List.iter (fun (_, v) -> Queue.push v queue) fields
        | _ ->
            ()
      in
      loop (current :: acc)
...
let descend_depth json =
  let rec descend acc current =
    ...
    match current with
    | `List items ->
        List.fold_left descend acc items
    | `Assoc fields ->
        List.fold_left (fun a (_, v) -> descend a v) acc fields
    | _ ->
        acc
  in
...
let test_compiled_regex ~ctx (compiled : Ast.compiled_regex) json =
  match json with
  | `String s ->
      `Bool (Re.execp compiled.regex s)
  | _ ->
      fail_invalid_type ~ctx "test" json
...
let match_compiled_regex ~ctx (compiled : Ast.compiled_regex) json =
  ...
  | _ ->
      fail_invalid_type ~ctx "match" json
...
let scan_compiled_regex ~ctx (compiled : Ast.compiled_regex) json =
  ...
  | _ ->
      fail_invalid_type ~ctx "scan" json
...
let capture_compiled_regex ~ctx (compiled : Ast.compiled_regex) json =
  ...
  | _ ->
      fail_invalid_type ~ctx "capture" json
...
let split_sep ~ctx sep json =
  ...
  | _ ->
      fail_invalid_type ~ctx "split" json
        ...
        List.filter_map
          (fun item ->
            ...
            | `String s ->
                Some s
            | `Null ->
                None (* null values are filtered out *)
            | `Bool true ->
                Some "true"
            | `Bool false ->
                Some "false"
            ...
            | `Float f ->
                if Float.is_integer f then
                  Some (Int.to_string (Float.to_int f))
                else
                  Some (Float.to_string f)
            | other ->
                Some
...
let slice ~ctx (start : int option) (finish : int option) (json : Json.t) =
  let start =
    ...
    | `String s, Some start when start > String.length s ->
        String.length s
    | `String s, Some start when start < 0 ->
        start + String.length s
    | `List l, Some start when start > List.length l ->
        List.length l
    | `List l, Some start when start < 0 ->
        start + List.length l
    | (`String _ | `List _), Some start ->
        start
    | (`String _ | `List _), None ->
        0
    | _ ->
        (* slice can't be parsed outside of List or String *) assert false
  in
  let finish =
    match (json, finish) with
    | `String s, None ->
        String.length s
    | `String s, Some end_ when end_ > String.length s ->
        String.length s
    | `String s, Some end_ when end_ < 0 ->
        end_ + String.length s
    | `List l, None ->
        List.length l
    | `List l, Some end_ when end_ > List.length l ->
        List.length l
    | `List l, Some end_ when end_ < 0 ->
        end_ + List.length l
    | (`String _ | `List _), Some end_ ->
        end_
    | _ ->
        (* slice can't be parsed outside of List or String *) assert false
  in
  match json with
  | `String _s when finish < start ->
      yield (`String "")
  | `String s ->
      yield (`String (String.sub s start (finish - start)))
  | `List _l when finish < start ->
      yield (`List [])
  | `List l ->
      let rec slice_loop acc i = function
        | [] ->
            List.rev acc
        | x :: xs ->
            if i >= start && i < finish then
              slice_loop (x :: acc) (i + 1) xs
            else
              slice_loop acc (i + 1) xs
      in
...
let rec interp ~ctx expression json : unit =
  ...
  | Key key ->
      yield (member ~ctx key json)
  | Index idx ->
      index ~ctx idx json
  | Slice (start, finish) ->
      slice ~ctx start finish json
  | Pipe (left, right) ->
      pipe ~ctx left right json
  | Update (path_expr, transform) ->
      update ~ctx path_expr transform json
  | Alternative (left, right) ->
      alternative ~ctx left right json
  | Comma (left_expr, right_expr) ->
      interp ~ctx left_expr json;
      interp ~ctx right_expr json
  | Operation (left, op, right) ->
      operation ~ctx left right op json
  | Assign (path, value_expr) ->
      assign ~ctx path value_expr json
  | List None ->
      yield (`List [])
  | List (Some expr) ->
...
and interp_fn0 ~ctx f json =
  ...
  | Trim ->
      trim ~ctx json
  | To_uppercase ->
      to_uppercase ~ctx json
  | To_lowercase ->
      to_lowercase ~ctx json
  | To_codepoints ->
      yield (to_codepoints ~ctx json)
  | From_codepoints ->
      yield (from_codepoints ~ctx json)
  | Sort ->
      yield (sort ~ctx json)
  | Unique ->
      yield (unique ~ctx json)
  | Reverse -> (
      match json with
      | `List l ->
          yield (`List (List.rev l))
      | _ ->
          fail_invalid_type ~ctx "reverse" json
    )
  | Min ->
      yield (min ~ctx json)
  | Max ->
      yield (max ~ctx json)
  | First ->
      first_of_array ~ctx json
  | Last ->
      last_of_array ~ctx json
  | Add ->
      add_array ~ctx json
  | Any ->
      yield (any ~ctx json)
  | All ->
      yield (all ~ctx json)
  | Flatten ->
      yield (flatten ~ctx max_int json)
  | Combinations ->
      combinations ~ctx json
  | Transpose ->
      yield (transpose ~ctx json)
  | Keys ->
      yield (keys ~ctx json)
  | To_entries ->
      yield (to_entries ~ctx json)
  | From_entries ->
      yield (from_entries ~ctx json)
  | Type ->
      yield (`String (Json.type_of json))
  | To_string ->
      yield (to_string ~ctx json)
  | To_number ->
      yield (to_number ~ctx json)
  | Length ->
      yield (length ~ctx json)
  | Byte_length ->
      yield (byte_length ~ctx json)
  | Numbers ->
      type_selector ~ctx "number" json
  | Strings ->
      type_selector ~ctx "string" json
  | Objects ->
      type_selector ~ctx "object" json
  | Arrays ->
      type_selector ~ctx "array" json
  | Booleans ->
      type_selector ~ctx "boolean" json
  | Nulls ->
      type_selector ~ctx "null" json
  | Iterables ->
      iterables_selector ~ctx json
  | Scalars ->
      scalars_selector ~ctx json
  | Values ->
      values_selector ~ctx json
  | Floor ->
      yield (floor ~ctx json)
  | Sqrt ->
      yield (sqrt ~ctx json)
  | Abs ->
      yield (abs_op ~ctx json)
  | Ceil ->
      yield (ceil_op ~ctx json)
  | Round ->
      yield (round_op ~ctx json)
  | Sin ->
      yield (math_fn ~ctx Float.sin "sin" json)
  | Cos ->
      yield (math_fn ~ctx Float.cos "cos" json)
  | Tan ->
      yield (math_fn ~ctx Float.tan "tan" json)
  | Asin ->
      yield (math_fn ~ctx Float.asin "asin" json)
  | Acos ->
      yield (math_fn ~ctx Float.acos "acos" json)
  | Atan ->
      yield (math_fn ~ctx Float.atan "atan" json)
  | Sinh ->
      yield (math_fn ~ctx Float.sinh "sinh" json)
  | Cosh ->
      yield (math_fn ~ctx Float.cosh "cosh" json)
  | Tanh ->
      yield (math_fn ~ctx Float.tanh "tanh" json)
  | Asinh ->
      yield (math_fn ~ctx Float.asinh "asinh" json)
  | Acosh ->
      yield (math_fn ~ctx Float.acosh "acosh" json)
  | Atanh ->
      yield (math_fn ~ctx Float.atanh "atanh" json)
  | Log ->
      yield (math_fn ~ctx Float.log "log" json)
  | Log10 ->
      yield (math_fn ~ctx Float.log10 "log10" json)
  | Log2 ->
      yield (math_fn ~ctx Float.log2 "log2" json)
  | Exp ->
      yield (math_fn ~ctx Float.exp "exp" json)
  | Exp2 ->
      yield (math_fn ~ctx Float.exp2 "exp2" json)
  | Expm1 ->
  ...
  | Truncate ->
      yield (math_fn ~ctx Float.trunc "truncate" json)
  | Is_normal ->
      yield (is_normal_op ~ctx json)
  | Is_nan ->
      yield (is_nan ~ctx json)
  | Nearbyint ->
      yield (math_fn ~ctx Float.round "nearbyint" json)
  | Logb ->
      yield (logb_op ~ctx json)
  | Infinite ->
  ...
  | Nan ->
      yield (`Float nan)
  | Now ->
      yield (`Float (Unix.gettimeofday ()))
  | Paths ->
      paths json
  | Leaf_paths ->
      leaf_paths json
  | Recurse ->
      yield_many (recurse_down json)
  | Recurse_down ->
      yield_many (recurse_down json)
  | Descend ->
      yield_many (descend_breadth_first json)
  | Dive ->
      yield_many (descend_depth json)
  | Empty ->
      ()
  | Not ->
      yield (Operators.not json)
  | Break ->
      break ()
  | Halt ->
      halt ()
  | Env ->
      yield (`Assoc (get_envs ()))
  | Debug ->
      debug json
  | Stderr ->
      stderr json
  | Builtins ->
      yield (builtins_list ())
  | To_local_time ->
      yield (to_local_time ~ctx json)
  | To_utc ->
      yield (to_utc ~ctx json)
  | To_unix ->
      yield (to_unix ~ctx json)
  | From_date ->
      yield (from_date ~ctx json)
  | Is_blank ->
      is_blank ~ctx json
  | Is_empty ->
      is_empty ~ctx Identity json
...
and interp_fn1 ~ctx fn1 json =
  ...
  | With_pattern (pattern_fn, compiled) -> (
      ...
      | Test ->
          yield (test_compiled_regex ~ctx compiled json)
      | Match ->
          match_compiled_regex ~ctx compiled json
      | Scan ->
          scan_compiled_regex ~ctx compiled json
      | Capture ->
          capture_compiled_regex ~ctx compiled json
    )
  | With_separator (sep_fn, sep) -> (
      match sep_fn with
      | Split ->
          yield (split_sep ~ctx sep json)
      | Join ->
          yield (join_sep ~ctx sep json)
      | Parse_date ->
          yield (parse_date_fn ~ctx sep json)
    )
  | With_expr (expr_fn, expr) -> (
      ...
      | Map ->
          map ~ctx expr json
      | Map_values ->
          map_values ~ctx expr json
      | Flat_map ->
          flat_map ~ctx expr json
      | Select ->
          select ~ctx expr json
      | Sort_by ->
          sort_by ~ctx expr json
      | Group_by ->
          group_by ~ctx expr json
      | Unique_by ->
          unique_by ~ctx expr json
      | Min_by ->
          min_by ~ctx expr json
      | Max_by ->
          max_by ~ctx expr json
      | Find ->
          find ~ctx expr json
      | Some_ ->
          some ~ctx expr json
      | Any_cond ->
          any_with_condition ~ctx expr json
      | All_cond ->
          all_with_condition ~ctx expr json
      | Pluck ->
          pluck ~ctx expr json
      | Partition ->
          partition ~ctx expr json
      | Flatten_n ->
          collect ~ctx expr json
          |> List.iter (fun depth ->
              match depth with
              | `Int d ->
                  yield (flatten ~ctx d json)
              | `Int64 d ->
                  yield (flatten ~ctx (Int64.to_int d) json)
              | `Float f ->
                  yield (flatten ~ctx (Int.of_float f) json)
              | _ ->
                  fail_invalid_type ~ctx "flatten: depth must be a number" depth
          )
      | Combinations_n ->
          combinations_n ~ctx expr json
      | Transpose_expr ->
          on_yield
            (fun () -> interp ~ctx expr json)
            ~then_:(fun json -> yield (transpose ~ctx json))
      | First_expr ->
          first_of_expr ~ctx expr json
      | Last_expr ->
          last_of_expr ~ctx expr json
      | Nth_array ->
          nth_array ~ctx expr json
      | Add_expr ->
          add_expr ~ctx expr json
      | Repeat ->
          repeat_expr ~ctx expr json
      | Binary_search ->
      ...
      | In ->
          in_ ~ctx json expr
      | With_entries ->
          with_entries ~ctx expr json
      | Delete ->
          del ~ctx expr json
      | Pick ->
          pick ~ctx expr json
      | Getpath ->
          getpath ~ctx expr json
      | Delpaths ->
          delpaths ~ctx expr json
      | Starts_with ->
          starts_with ~ctx expr json
      | Ends_with ->
          ends_with ~ctx expr json
      | Index_of ->
          index_of ~ctx expr json
      | Last_index_of ->
          last_index_of ~ctx expr json
      | Indices ->
          indices ~ctx expr json
      | Inside ->
          inside ~ctx expr json
      | Trim_start ->
          trim_start_impl ~ctx expr json
      | Trim_end ->
          trim_end_impl ~ctx expr json
      | Contains ->
          contains ~ctx expr json
      | Walk ->
          walk_tree ~ctx expr json
      | Path ->
          path_of ~ctx expr json
      | Paths_filter ->
          paths_filter ~ctx expr json
      | Recurse_expr ->
          let rec loop value =
            yield value;
            on_yield
          ...
          loop json
      | Find_all ->
          find_all ~ctx expr json
      | Find_first ->
          find_first ~ctx expr json
      | Paths_to ->
          paths_to ~ctx expr json
      | Is_empty_expr ->
          is_empty ~ctx expr json
      | Error_msg ->
          error_msg ~ctx (Some expr) json
      | Halt_error_n -> (
          match collect ~ctx expr json with
          | [ `Int n ] ->
              halt ~code:n ()
          | [ `Int64 n ] ->
              halt ~code:(Int64.to_int n) ()
          | [ `Float f ] ->
              halt ~code:(Float.to_int f) ()
          | [ v ] ->
      ...
      | Debug_msg ->
          debug_msg ~ctx (Some expr) json
      | Assert_simple ->
          assert_ ~ctx expr None json
    )
...
and interp_fn2 ~ctx f e1 e2 json =
  ...
  | While ->
      while_loop ~ctx e1 e2 json
  | Until ->
      until_loop ~ctx e1 e2 json
  | Limit -> (
      match collect ~ctx e1 json with
      | [ `Int n ] ->
          limit ~ctx n e2 json
      | [ `Int64 n ] ->
          limit ~ctx (Int64.to_int n) e2 json
      | [ `Float f ] ->
          limit ~ctx (Float.to_int f) e2 json
      | [ v ] ->
  ...
  | Skip -> (
      ...
      | [ `Int n ] ->
          skip ~ctx n e2 json
      | [ `Int64 n ] ->
          skip ~ctx (Int64.to_int n) e2 json
      | [ `Float f ] ->
          skip ~ctx (Float.to_int f) e2 json
      | [ v ] ->
  ...
  | Recurse_with ->
      ...
      yield_many results
  | Any_gen ->
      any_with_generator ~ctx e1 e2 json
  | All_gen ->
      all_with_generator ~ctx e1 e2 json
  | Assert_msg ->
      assert_ ~ctx e1 (Some e2) json
  | Raise ->
      raise_error ~ctx e1 e2 json
  | Setpath ->
      setpath ~ctx e1 e2 json
  | Nth ->
      nth ~ctx e1 e2 json
  | Atan2 ->
      atan2_op ~ctx e1 e2 json
  | Copysign ->
      copysign_op ~ctx e1 e2 json
  | Ldexp ->
      ldexp_op ~ctx e1 e2 json
  | Fdim ->
      fdim_op ~ctx e1 e2 json
  | Remainder ->
      remainder_op ~ctx e1 e2 json
  | Scalbn ->
      scalbn_op ~ctx e1 e2 json
  | Pow2 ->
      pow2_op ~ctx e1 e2 json
...
and map_values ~ctx (expr : expression) (json : Json.t) =
  ...
  | `Assoc entries ->
      ...
      yield (`Assoc mapped)
  | _ ->
      fail_invalid_type ~ctx "map_values" json
...
and min_by ~ctx expr json =
  ...
  | `List l ->
      ...
      yield min_elem
  | _ ->
      fail_invalid_type ~ctx "min_by" json
...
and max_by ~ctx expr json =
  ...
  | `List l ->
      ...
      yield max_elem
  | _ ->
      fail_invalid_type ~ctx "max_by" json
  ...
  let interp_field (left_expr, right_expr) =
    ...
    List.concat_map
      (fun k ->
        match k with
        | `String k_str ->
            List.map (fun v -> (k_str, v)) values_res
        | _ ->
...
and flat_map ~ctx expr json =
  ...
  | `List _ ->
      Runtime_error.empty_array "flat_map"
  | _ ->
      fail_invalid_type ~ctx "flat_map" json
...
and find ~ctx expr json =
  ...
  | `List list ->
      let rec find_first = function
        ...
        | x :: xs -> (
            ...
            | [ `Bool false ] ->
                find_first xs
            | [ other ] ->
                if other = `Null || other = `Bool false then
                  find_first xs
                else
                  yield x
            | _ ->
      ...
      find_first list
  | _ ->
      fail_invalid_type ~ctx "find" json
...
and some ~ctx expr json =
  ...
  | `List list ->
      ...
      check_some list
  | _ ->
      fail_invalid_type ~ctx "some" json
  ...
  | `List list ->
      ...
      let rec check_all = function
        ...
        | x :: xs -> (
            try
              let results = collect ~ctx expr x in
              if List.for_all is_truthy results then
                check_all xs
              else
                yield (`Bool false)
  ...
  let rec check = function
    ...
    | x :: xs -> (
        try
          ...
          if List.exists is_truthy cond_results then
            yield (`Bool true)
          else
            check xs
        with _ -> check xs
...
and all_with_generator ~ctx gen cond json =
  ...
  let rec check = function
    ...
    | x :: xs -> (
        try
          let cond_results = collect ~ctx cond x in
          if List.for_all is_truthy cond_results then
            check xs
          else
            yield (`Bool false)
        with _ -> yield (`Bool false)
...
and path_of ~ctx expr json =
  let rec extract_paths current_path expression value =
    ...
    | Index indices when indices = [] -> (
        match value with
        | `List l ->
            List.mapi (fun i _ -> current_path @ [ `Int i ]) l
        | `Assoc fields ->
        ...
        List.concat_map
          (fun key_or_idx ->
            ...
            | `List _, `Int idx ->
                [ current_path @ [ `Int idx ] ]
            | `List _, `Float f ->
                [ current_path @ [ `Int (Float.to_int f) ] ]
            | _ ->
                []
          )
          keys_or_indices
...
and in_ ~ctx json expr =
  ...
  | [ container ] -> (
      ...
      | `Int n, `List l ->
          yield (`Bool (n >= 0 && n < List.length l))
      | `String key, `Assoc list ->
          yield (`Bool (List.mem_assoc key list))
      | _ ->
          fail_invalid_type ~ctx "in" json
    )
  | _ ->
...
and if_then_else ~ctx cond if_branch else_branch json =
  on_yield
    ...
    ~then_:(function
      | `Bool false | `Null ->
          interp ~ctx else_branch json
      | _ ->
          interp ~ctx if_branch json
      )

and call_function ~ctx fname args json =
  match List.assoc_opt fname ctx.fns with
  | None ->
      Runtime_error.undefined_function ~name:fname
  | Some { params; body } ->
        ...
        let filter_params, filter_args, value_params, value_args =
          List.fold_left2
            (fun (fp, fa, vp, va) param arg ->
              if is_value_param param then
                (fp, fa, param :: vp, arg :: va)
              else
                (param :: fp, arg :: fa, vp, va)
            )
            ([], [], [], []) params args
        in
...
and ends_with ~ctx expr json =
  ...
  |> List.iter (fun pattern ->
      ...
      | `String s, `String suffix ->
          yield (`Bool (String.ends_with ~suffix s))
      | `String _, other ->
...
and with_entries ~ctx expr json =
  ...
  | _ ->
      fail_invalid_type ~ctx "to_entries failed" json
...
and contains ~ctx expr json =
  ...
  match collect ~ctx expr json with
  | [ needle ] ->
      yield (`Bool (value_contains json needle))
  | _ ->
...
and index_of ~ctx expr json =
  ...
  |> List.iter (fun needle ->
      ...
      | _ ->
          fail_invalid_type ~ctx "index" json
  )
...
and last_index_of ~ctx expr json =
  ...
  |> List.iter (fun needle ->
      ...
      | _ ->
          fail_invalid_type ~ctx "last_index" json
  )
...
and indices ~ctx expr json =
  ...
  |> List.iter (fun needle ->
      ...
      | _ ->
          fail_invalid_type ~ctx "indices" json
  )
...
and inside ~ctx expr json =
  ...
  |> List.iter (fun container ->
      ...
      | `Assoc _, `Assoc _ ->
          yield (`Bool (Json.contains json container))
      | _ ->
          fail_invalid_type ~ctx "inside" json
  )
...
and trim ~ctx json =
  match json with
  | `String s ->
      yield (`String (String.trim s))
  | _ ->
      fail_invalid_type ~ctx "trim" json
...
and combinations ~ctx json =
  ...
  | _ ->
      fail_invalid_type ~ctx "combinations" json
...
and add_array ~ctx json =
  ...
  | `List (first :: rest) ->
      ...
      yield sum
  | _ ->
      fail_invalid_type ~ctx "add" json
...
and to_uppercase ~ctx json =
  match json with
  | `String s ->
      yield (`String (String.uppercase_ascii s))
  | _ ->
      fail_invalid_type ~ctx "to_uppercase" json

and to_lowercase ~ctx json =
  match json with
  | `String s ->
      yield (`String (String.lowercase_ascii s))
  | _ ->
      fail_invalid_type ~ctx "to_lowercase" json
...
and binary_search ~ctx expr json =
  ...
  | _ ->
      fail_invalid_type ~ctx "binary_search" json

and first_of_array ~ctx json =
  match json with
  | `List [] ->
      Runtime_error.empty_array "first"
  | `List (hd :: _) ->
      yield hd
  | _ ->
      fail_invalid_type ~ctx "first" json
...
and nth ~ctx n_expr expr json =
  ...
  | Some n ->
      ...
      if n >= 0 && n < len then
        yield (List.nth results n)
      else
        Runtime_error.index_out_of_bounds ~index:n ~length:len ~value:json
  | None ->
      fail_invalid_type ~ctx "nth: first argument must be a number" json
  ...
  | Some n, `List items ->
      ...
      let actual_n =
        if n < 0 then
          ...
          n
      in
      if actual_n >= 0 && actual_n < len then
        yield (List.nth items actual_n)
      else
        Runtime_error.index_out_of_bounds ~index:n ~length:len ~value:json
  | Some _, _ ->
      fail_invalid_type ~ctx "nth" json
  | None, _ ->
      fail_invalid_type ~ctx "nth: argument must be a number" json
...
and group_by ~ctx expr json =
  ...
  | `List l ->
      ...
      yield (`Assoc result)
  | _ ->
      fail_invalid_type ~ctx "group_by" json
...
and recurse_with_cond ~ctx f cond json =
  let rec loop acc current =
    ...
    | [ `Bool true ] -> (
        ...
        match next_res with
        | [ next ] ->
            loop acc_with_current next
        | _ ->
            List.rev acc_with_current
      )
    | [ `Bool false ] ->
        List.rev acc
    | _ ->
...
and walk_tree ~ctx expr json =
  let rec walk json =
    let walked_json =
      match json with
      | `List l ->
          `List (List.map walk l)
      | `Assoc obj ->
          `Assoc (List.map (fun (k, v) -> (k, walk v)) obj)
      | other ->
          other
    in
...
and try_catch ~ctx expr handler finally_expr json =
  let run_finally () =
    ...
    | None ->
        ()
    | Some cleanup ->
        ignore (collect ~ctx cleanup json)
  in
...
and delete_path value path_components =
  let rec del_at value = function
    ...
    | [ `String key ] -> (
        match value with
        | `Assoc fields ->
            `Assoc (List.filter (fun (k, _) -> k <> key) fields)
        | _ ->
            value
      )
    | [ ((`Int _ | `Int64 _ | `Big_int _ | `Float _ | `Decimal _) as num) ] -> (
        ...
        match value with
        | `List items ->
            `List (List.filteri (fun i _ -> i <> idx) items)
        | _ ->
            value
      )
    | `String key :: rest -> (
        ...
        | `String key :: rest -> (
            ...
            | `Assoc fields ->
                ...
                if exists then
                  ...
                  `Assoc (fields @ [ (key, set_at `Null rest) ])
            | `Null ->
                `Assoc [ (key, set_at `Null rest) ]
            | _ ->
                value
          )
        | ((`Int _ | `Int64 _ | `Big_int _ | `Float _ | `Decimal _) as num)
  ...
  let rec extract_path_expr = function
    ...
    | Pipe (left, right) ->
        ...
        List.concat_map
          ...
          left_paths
    | Comma (left, right) ->
        extract_path_expr left @ extract_path_expr right
...
and slice_expr ~ctx (start_expr : expression option)
  ...
  let eval_to_int expr =
    ...
    | [ `Float f ] ->
        Float.to_int f
    | _ ->
        failwith "slice index must evaluate to a number"
  in
...
and pluck ~ctx expr json =
  ...
  | `List items ->
      ...
      yield (`List results)
  | _ ->
      fail_invalid_type ~ctx "pluck" json
      ...
      List.iter
        (fun item ->
          ...
          if List.exists is_truthy results then
            ...
            non_matching := item :: !non_matching
        )
        items;
      yield
        (`List [ `List (List.rev !matching); `List (List.rev !non_matching) ])
  | _ ->
      fail_invalid_type ~ctx "partition" json
...
and is_empty ~ctx expr json =
  ...
  | Identity -> (
      ...
      | `String _ ->
          yield (`Bool false)
      | _ ->
          fail_invalid_type ~ctx "is_empty" json
    )
  | _ -> (
      (* with argument: check if expression produces no output (jq semantics) *)
      match collect ~ctx expr json with
...
and is_blank ~ctx json =
  ...
  | _ ->
      fail_invalid_type ~ctx "is_blank" json
    ...
    List.concat_map
      (fun value ->
        ...
        | [ `Bool false ] | [ `Null ] | [] ->
            []
        | results ->
            List.filter (fun r -> r <> `Null) results
      )
      all_values
  in
  yield (`List matches)
