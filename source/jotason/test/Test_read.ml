...
let parse_null () =
  check_parse "null" Fixtures.null_json Fixtures.null_value ()

let parse_true () =
  check_parse "true" Fixtures.true_json Fixtures.true_value ()

let parse_false () =
  check_parse "false" Fixtures.false_json Fixtures.false_value ()

let parse_zero () =
  check_parse "zero" Fixtures.zero_json Fixtures.zero_value ()

let parse_positive_int () =
  check_parse "positive int" Fixtures.positive_int_json Fixtures.positive_int_value ()

let parse_negative_int () =
  check_parse "negative int" Fixtures.negative_int_json Fixtures.negative_int_value ()
...
let parse_negative_float () =
  check_parse "negative float" Fixtures.negative_float_json Fixtures.negative_float_value ()

let parse_exp_float () =
  check_parse "exponential float" Fixtures.exp_float_json Fixtures.exp_float_value ()

let parse_exp_negative () =
  check_parse "negative exponent" Fixtures.exp_negative_json Fixtures.exp_negative_value ()

let parse_exp_positive () =
  check_parse "positive exponent" Fixtures.exp_positive_json Fixtures.exp_positive_value ()

let parse_zero_point () =
  check_parse "0.0" Fixtures.zero_point_json Fixtures.zero_point_value ()

let parse_leading_zero_float () =
  check_parse "0.123" Fixtures.leading_zero_float_json Fixtures.leading_zero_float_value ()

let parse_empty_string () =
  check_parse "empty string" Fixtures.empty_string_json Fixtures.empty_string_value ()

let parse_simple_string () =
  check_parse "simple string" Fixtures.simple_string_json Fixtures.simple_string_value ()

let parse_escaped_quote () =
  check_parse "escaped quote" Fixtures.escaped_quote_json Fixtures.escaped_quote_value ()

let parse_escaped_backslash () =
  check_parse "escaped backslash" Fixtures.escaped_backslash_json Fixtures.escaped_backslash_value ()

let parse_escaped_slash () =
  check_parse "escaped slash" Fixtures.escaped_slash_json Fixtures.escaped_slash_value ()

let parse_escaped_backspace () =
  check_parse "escaped backspace" Fixtures.escaped_backspace_json Fixtures.escaped_backspace_value ()

let parse_escaped_formfeed () =
  check_parse "escaped formfeed" Fixtures.escaped_formfeed_json Fixtures.escaped_formfeed_value ()

let parse_escaped_newline () =
  check_parse "escaped newline" Fixtures.escaped_newline_json Fixtures.escaped_newline_value ()

let parse_escaped_carriage () =
  check_parse "escaped carriage return" Fixtures.escaped_carriage_json Fixtures.escaped_carriage_value ()
...
let parse_unicode_basic () =
  check_parse "unicode basic" Fixtures.unicode_basic_json Fixtures.unicode_basic_value ()

let parse_unicode_euro () =
  check_parse "unicode euro" Fixtures.unicode_euro_json Fixtures.unicode_euro_value ()

let parse_unicode_snowman () =
  check_parse "unicode snowman" Fixtures.unicode_snowman_json Fixtures.unicode_snowman_value ()

let parse_unicode_surrogate () =
  check_parse "unicode surrogate pair" Fixtures.unicode_surrogate_json Fixtures.unicode_surrogate_value ()

let parse_utf8_direct () =
  check_parse "direct utf8" Fixtures.utf8_direct_json Fixtures.utf8_direct_value ()

let parse_utf8_emoji () =
  check_parse "direct utf8 emoji" Fixtures.utf8_emoji_json Fixtures.utf8_emoji_value ()

let parse_empty_array () =
  check_parse "empty array" Fixtures.empty_array_json Fixtures.empty_array_value ()

let parse_single_array () =
  check_parse "single element array" Fixtures.single_array_json Fixtures.single_array_value ()

let parse_mixed_array () =
  check_parse "mixed array" Fixtures.mixed_array_json Fixtures.mixed_array_value ()

let parse_nested_array () =
  check_parse "nested array" Fixtures.nested_array_json Fixtures.nested_array_value ()

let parse_deeply_nested () =
  check_parse "deeply nested" Fixtures.deeply_nested_json Fixtures.deeply_nested_value ()

(* === Object tests === *)

let parse_empty_object () =
  check_parse "empty object" Fixtures.empty_object_json Fixtures.empty_object_value ()

let parse_single_object () =
  check_parse "single key object" Fixtures.single_object_json Fixtures.single_object_value ()

let parse_multi_object () =
  check_parse "multi key object" Fixtures.multi_object_json Fixtures.multi_object_value ()

let parse_nested_object () =
  check_parse "nested object" Fixtures.nested_object_json Fixtures.nested_object_value ()

let parse_object_with_array () =
  check_parse "object with array" Fixtures.object_with_array_json Fixtures.object_with_array_value ()

let parse_array_of_objects () =
  check_parse "array of objects" Fixtures.array_of_objects_json Fixtures.array_of_objects_value ()

let parse_key_with_space () =
  check_parse "key with space" Fixtures.key_with_space_json Fixtures.key_with_space_value ()

let parse_key_with_unicode () =
  check_parse "key with unicode" Fixtures.key_with_unicode_json Fixtures.key_with_unicode_value ()

let parse_empty_key () =
  check_parse "empty key" Fixtures.empty_key_json Fixtures.empty_key_value ()

let parse_duplicate_keys () =
  check_parse "duplicate keys" Fixtures.duplicate_keys_json Fixtures.duplicate_keys_value ()

(* === Whitespace tests === *)

let parse_whitespace () =
  check_parse "extra whitespace" Fixtures.whitespace_json Fixtures.whitespace_value ()

let parse_newlines () =
  check_parse "newlines" Fixtures.newlines_json Fixtures.newlines_value ()

let parse_tabs () =
  check_parse "tabs" Fixtures.tabs_json Fixtures.tabs_value ()
...
let fail_missing_comma_object () =
  Alcotest.check_raises "missing comma in object"
    (Json.Json_error "Line 1, bytes 7-13:\nExpected ',' or '}' but found '\"b\":2}'")
    (parse {|{"a":1 "b":2}|})
...
let single_json =
  [
    ...
    ("from_string_fail_nested_unterminated", `Quick, from_string_fail_nested_unterminated);
    ("from_string_fail_unterminated_structure", `Quick, from_string_fail_unterminated_structure);
    ("from_string_fail_unstarted_structure", `Quick, from_string_fail_unstarted_structure);
    ("from_string_fail_unstarted_object", `Quick, from_string_fail_unstarted_object);
