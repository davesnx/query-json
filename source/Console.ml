...
module Errors = struct
  ...
  let extractExn (str : string) =
    ...
    | Some firstIndex, Some lastIndex ->
        let first = firstIndex + 2 in
        let length = lastIndex - firstIndex - 1 - 2 in
        String.sub str first length
    | _, _ -> str

  let make ~input ~(start : Lexing.position) ~(end_ : Lexing.position) =
    let pointerRange = String.make (end_.pos_cnum - start.pos_cnum) '^' in
  ...
  let renamed f name =
    Formatting.singleQuotes f ^ " is not valid in q, use "
    ^ Formatting.singleQuotes name
    ^ " instead"

  let notImplemented f = Formatting.singleQuotes f ^ " is not implemented"
