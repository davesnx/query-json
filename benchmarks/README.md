...
The `event_member_first` row reads `benchmarks/big.json` before timing and wraps its array as `{"rows":<array>,"tail":0}`. The query is `.rows[] | .id`. It compares `Core.When_ready` (`W`) with `Core.After_validation` (`A`) through `Core.run_input_iter` on the same retained `Core.String` source.

Before timing, the benchmark requires a compiled plan and a `Member_items ("rows", ...)` cut through `Execution.For_test.stream_cut`. It collects both policies' complete rendered output arrays, requires exact equality, and requires the first output to be `1`.

Each paired sample makes one fresh call per policy and alternates their order. `--samples` sets the number of pairs. `--warmup` is capped at three calls per policy. `--iterations` does not apply to this table. Immediately before each call, the benchmark runs `Gc.full_major ()`, reads `Gc.allocated_bytes ()`, and reads `Unix.gettimeofday ()`. The first emit callback records elapsed nanoseconds and allocated bytes before it checks the result. The call continues to EOF and checks every rendered output, its order, and the final count against the expected array. An error or mismatch fails the run.

The measurement includes query parsing and preparation, JSON parsing to the first complete child, residual execution, and rendering. `A` also parses and validates the rest of the document before its first output. Fixture load and generation, process startup, stdout, and result checks are excluded. The source string and expected output array remain retained. This measures the first library callback, not the first byte observed from a CLI process.

The row reports medians for `W first ns`, `A first ns`, `W first B`, and `A first B`, followed by time delta, allocation delta, and samples. Deltas are `(W / A - 1) * 100`. **B is allocation volume to the first callback, not peak RSS or total allocation through EOF.** There is no performance threshold.
...
Each timed command starts a fresh CLI process, reads the same file, and sends stdout to `/dev/null`. These measurements include startup, parsing, execution, rendering, and output handling. Default mode validates first and collects output atomically. Ready mode flushes each output, so this comparison also includes different result collection and output handling costs. It does not isolate only the library delivery policy.
...
- The prepared-execution and prepared-sink tables exclude query parsing, preparation, and rendering. The one-shot, input-inclusive, and first-result tables include them. The input-inclusive and first-result tables include JSON parsing. First-result timing ends at the first callback, before result checks and, for `W`, before full validation. All tables exclude fixture generation, result checks, startup, and stdout. None measures end-to-end CLI latency.
