...
module Query_cmd = struct
  let run query payload verbose debug no_color raw_output help_category =
    ...
    | None -> (
        ...
        | Some query -> (
            let output =
              ...
              in
              Core.run ~debug ~colorize ~verbose ~raw:raw_output
  ...
  let term =
    ...
    in
    const run $ query $ json $ verbose $ debug $ no_color $ raw_output
    $ help_category
end

let () =
  (* Check if first arg is "repl" to dispatch to subcommand *)
  ...
  if is_repl_command then
    ...
    in
    (* Remove "repl" from argv for proper parsing *)
    let argv =
      Array.concat
